...
> agent-core-service 与 tool-service 之间通过 RabbitMQ 事件驱动协作，任务/步骤全程落库（持久化驱动），支持失败重试与断点续跑。P3a 在此基础上为报销任务增加角色化固定流水线；审批工单闭环仍属于 P3b。
...
任务 `agent_task.status`：`PENDING → RUNNING → SUCCESS / FAILED / APPROVAL_PENDING / REJECTED`。

- `APPROVAL_PENDING` 是 P3a 结果分支 `NEED_REVIEW` 的待人工复核标记，并不代表当前已经生成审批工单。
- `REJECTED` 为 P3b 审批动作预留状态，当前没有 approve/reject/amend/terminate 接口。
...
P3b 将在后续增加审批工单、审批动作和审计留痕，不改变上述任务/工具 MQ 契约。
...
| `SUCCESS/FAILED/APPROVAL_PENDING/REJECTED` | 400 拒绝（任务已终结；待审批任务须等待 P3b） |
