...
| 待办 | 说明 |
|---|---|
| 外部发票查验对接 | 官方/商业查验 API（按次付费，待商务决策；R8-1） |
| ~~主动通知~~ | ✅ **已完成（P3.8 R8-2）**：站内信 + Webhook（outbox 投递 + HMAC 签名 + 退避重试 + 投递台账），MQ 死信告警已接通知通道 |
| ~~进度百分比与预计等待~~ | ✅ **已完成（P3.8 R8-3）**：`GET /api/v1/tasks/{id}/progress` |
| 报销单列表轮询 | `reimbursement/list.vue` 无轮询，RUNNING 需手刷（归前端阶段 R8-4；站内信前端页同属该阶段） |
