...
> **对克隆本仓的贡献者，本文就是规范正文**（无需其他文件即可对齐）。
>
> 仓库内另有一份更细的强制规范 `AGENTS.md` / `CLAUDE.md`，**被 `.gitignore` 有意排除、不入版本库**
> （提交 `6502641` 的决策：本地工具文件不对外发布）。其中可公开的部分已全部收敛进本文；
> 留本地的只是策略性内容。**因此不要再把 `AGENTS.md` / `CLAUDE.md` 当作对外文档入口来引用。**
>
> P3.8 R6-7 落地；P3.8 R7-9（D-15）补入上述公开边界说明。
